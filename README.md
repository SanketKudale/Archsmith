...
Shared visual values live in `.archsmith/design_tokens.json`; colors, spacing, radii, and font sizes are selectable in the inspector and generate one typed `ArchsmithDesignTokens` class. `.archsmith/localization.json` provides searchable `$i18n.key` values with locale fallback. The Asset Image component lists existing raster files under `assets/`, previews them locally, and requires a semantic label unless marked decorative. Buttons also support generated semantics and tooltips.
